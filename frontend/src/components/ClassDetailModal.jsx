...
import { X, Clock, MapPin, User, Tag, Calendar, StickyNote, Edit2, Trash2, BookOpen } from 'lucide-react';
...
export default function ClassDetailModal({ isOpen, onClose, cls, status, onEdit, onDelete }) {
  ...
  const classNotes = cls ? notes.filter(n => n.classId === cls._id) : [];
  const duration = cls ? formatDuration(cls.startTime, cls.endTime) : '';
        ...
        {isOpen && cls && (
          ...
          >
            <motion.div 
              ...
              className="relative bg-white dark:bg-[#0d1222] rounded-3xl border border-gray-200/90 dark:border-gray-800/90 w-full max-w-lg shadow-2xl overflow-hidden"
            >
              {/* Colored accent line */}
              <div
              ...
              <div className="px-6 space-y-3 max-h-[58vh] overflow-y-auto pb-6">
                ...
                <div className="p-3.5 rounded-2xl bg-purple-50/70 dark:bg-purple-950/25 border border-purple-100 dark:border-purple-900/40 flex items-center justify-between gap-3">
                  ...
                  <motion.button
                    ...
                    className="px-3.5 py-1.5 rounded-xl text-xs font-semibold bg-purple-600 hover:bg-purple-700 text-white transition shadow-sm"
                  >
